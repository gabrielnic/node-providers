...
interface GraphProps {
    ...
    highlightNodeId?: string; // New prop to indicate which node to highlight.
}
...
const Graph: React.FC<GraphProps> = ({
    ...
    useEffect(() => {
        ...
        function tick() {
            linkSelection
                .attr("x1", (d) => (typeof d.source !== "string" ? (d.source as GraphNode).x || 0 : 0))
                .attr("y1", (d) => (typeof d.source !== "string" ? (d.source as GraphNode).y || 0 : 0))
                .attr("x2", (d) => (typeof d.target !== "string" ? (d.target as GraphNode).x || 0 : 0))
                .attr("y2", (d) => (typeof d.target !== "string" ? (d.target as GraphNode).y || 0 : 0));
            ...
            .call(
                (drag<SVGCircleElement, GraphNode, GraphNode>() as any)
                    ...
                    .on("end", (event: D3DragEvent<SVGCircleElement, GraphNode, GraphNode>, d: GraphNode) => {
                        ...
                        // Comment out these lines to fix node position after drag:
                        // d.fx = null;
                        // d.fy = null;
                    })
            )
    ...
    // New effect: highlight the searched node if highlightNodeId is provided
    useEffect(() => {
        ...
        if (!node.empty()) {
            ...
            node.transition().duration(500)
                .attr("r", 20)
                .attr("stroke", "red");
            // Optionally, pan to that node here
        }
    }, [highlightNodeId]);
